 ...
 * @version 1.0.0
 */
public class FirForestOverworldBiomes {
    // 一个辅助的函数用于根据温度判断段生物群系的天空的颜色
    protected static int calculateSkyColor(float pTemperature) {
        return rgbToColor(120, 169, 255);
    }
    // 一个构造方法 用于返回一个biome实例
    private static Biome FirForestOverworldBiomes(
    ...
    {
        return FirForestOverworldBiomes(pHasPercipitation, pTemperature, pDownfall, rgbToColor(121, 192, 224), rgbToColor(121, 192, 224), rgbToColor(82, 191, 120), null, pMobSpawnSettings, pGenerationSettings, pBackgroundMusic);
    }
    ...
    public static Biome firForestBiome(HolderGetter<PlacedFeature> pPlacedFeatures,
                                             HolderGetter<ConfiguredWorldCarver<?>> pWorldCarvers)
    {
        ...
        // 🌳 树木（重点）
        genBuilder.addFeature(GenerationStep.Decoration.VEGETAL_DECORATION, ModPlacedFeatures.GREEN_FIR_PLACED);
        genBuilder.addFeature(GenerationStep.Decoration.VEGETAL_DECORATION, ModPlacedFeatures.ORANGE_FIR_PLACED);
